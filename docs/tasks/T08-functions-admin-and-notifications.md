...
## Context
- **Sandbox note (from T07 review):** in the Claude Code sandbox, the Firebase CLI routes the functions emulator's trigger registration (`POST 127.0.0.1:8080/emulator/v1/.../eventarcTrigger`) through the HTTPS proxy even though `NO_PROXY` already lists `127.0.0.1`, and it fails with 403. Unsetting the proxy is not allowed in the sandbox. Consequences: locally, run the `functions`-only emulator checks (load check, callable smoke test), unit tests, and e2e flows that don't need Firestore triggers. Tests that need Firestore triggers are verified in GitHub Actions CI (no proxy there); the orchestrator checks that CI job before accepting the task. If the whole `test:e2e` cannot start locally because of this, STOP and report rather than working around the proxy.
