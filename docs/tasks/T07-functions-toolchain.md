...
- **firebase-admin 14 removed the legacy namespaced API** (commit "Remove Deprecated Legacy Namespace Support"; also "Drop support for Node.js 18 and 20", engines `>=22`). Verified: `tsc` fails on `admin.firestore()`, `admin.messaging()`, `admin.firestore.FieldValue` and `admin.messaging.MulticastMessage`. Replace them with the modular entry points, which admin 12 already supported with identical semantics: the same default app, `FieldValue` class and `MulticastMessage` type. `messaging.sendEachForMulticast` (:172) still exists; admin 13 removed only the legacy `sendToDevice`/`sendAll`/`sendMulticast`, which are not used.
