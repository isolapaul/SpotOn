...
| BUG-26 | The hero's Close and favourite (heart) buttons sit inside the clickable hero, so tapping them also opens the fullscreen gallery; because the panel never unmounts, the next spot opens straight into the gallery (found during T21) | `SpotDetailsPanel.tsx` hero | T28 |
