...
| BUG-09 | `isFavorite` is copied from props once and never re-synced | `SpotDetailsPanel.tsx:106`, `SpotInfoWindow.tsx:28` | T28 |
...
| BUG-16 | Timers without cleanup (can set state after unmount) | `page.tsx:112,134`, `LanguageSelector.tsx:17`, `NotificationPrompt.tsx:36` | T29 |
...
| BUG-19 | `MapView` rebuilds `L.divIcon` and `new Date()` for every marker on every render | `MapView.tsx:248,255` | T28/T29 (memoize) |
